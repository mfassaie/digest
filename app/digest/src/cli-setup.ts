...
// Locate the packaged dist/docker/ build context (generated by the build
// from @digest/docker's dist/docker). Works from dist/ (published) and
// from src/ (dev via tsx): dist/ is a sibling in both layouts.
function dockerContext(): string {
  const here = dirname(fileURLToPath(import.meta.url));
  return join(here, '..', 'dist', 'docker');
}
