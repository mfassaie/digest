...
import {
  normaliseUrl,
  getCacheDir, writeContent, readCacheMeta, writeCacheMeta,
  readMarkdown, readStructure,
  extractSection, formatOutline,
  DockerUnavailableError,
  type CacheMeta, type ContainerFetchResponse, type ContainerTransport,
  type ReadEngine,
} from '@digest/shared';
...
// Injected dependencies: the container transport is an interface defined in
// shared ({ensure, fetch}); the app wires @digest/docker's implementation in,
// so this package never depends on docker.
...
export async function handleGet(
  ...
  let baseUrl: string;
  try {
    ({ baseUrl } = await deps.transport.ensure());
  ...
  const res = await deps.transport.fetch(baseUrl, {
    url: normUrl,
    timeoutSeconds,
    rawOnly,
    validators: existing
...
export function createServer(deps: ServerDeps, version = '0.0.0') {
